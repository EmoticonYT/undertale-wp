#!/usr/bin/env python3
"""
Windows Phone 8.1 AppX Packager for Butterscotch (Undertale)
Creates a compliant .appx package containing the binary, assets, and Undertale data.
"""
...
def package_appx(project_dir, binary_path, output_appx_path):
    print("=== Packaging Windows Phone 8.1 AppX ===")
    staging_dir = os.path.join(project_dir, "build_appx_staging")
    ...
    # 1. Manifest
    manifest_src = os.path.join(project_dir, "winphone", "Package.appxmanifest")
    ...
    if os.path.exists(binary_path):
        ...
        print(f"[NOTE] Binary not found at {binary_path}. Generating dummy/placeholder binary for packaging test.")
        binary_dst = os.path.join(staging_dir, "Butterscotch.exe")
    ...
    # 3. Assets
    assets_src_dir = os.path.join(project_dir, "winphone", "Assets")
    ...
    for root_dir, _, files in os.walk(assets_src_dir):
        for f in files:
            ...
            rel_asset = os.path.relpath(src_f, project_dir + "/winphone")
    ...
    if os.path.exists(data_dir):
        ...
        for root_dir, _, files in os.walk(data_dir):
            for f in files:
                # Exclude build artifacts or DLLs that are not game assets
    ...
    with zipfile.ZipFile(output_appx_path, "w", zipfile.ZIP_DEFLATED) as zip_out:
        # AppxBlockMap.xml and AppxManifest.xml must be placed at the root
...
if __name__ == "__main__":
    ...
    bin_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(base_dir, "winphone", "ARM", "Release", "Butterscotch.exe")
    out_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(base_dir, "dist", "Undertale_WP8.1.appx")
